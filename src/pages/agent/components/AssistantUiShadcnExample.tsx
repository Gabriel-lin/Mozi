...
const Thread: FC = () => {
  return (
    ...
    >
      ...
      >
        <AuiIf condition={(s) => threadOf(s).isEmpty}>
          <ThreadWelcome />
        </AuiIf>
...
const ComposerAction: FC = () => {
  return (
    <div className="aui-composer-action-wrapper relative flex items-center justify-between">
      <ComposerAddAttachment />
      <AuiIf condition={(s) => !threadOf(s).isRunning}>
...
const AssistantMessage: FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* 与上游 `GroupedParts` 视觉等价：`MessagePrimitive.Parts` 为 @assistant-ui/react@0.12 稳定 API */}
    ...
    >
      <ActionBarPrimitive.Copy asChild>
        <TooltipIconButton tooltip="Copy">
          <AuiIf condition={(s) => messageOf(s).isCopied}>
            <CheckIcon />
          </AuiIf>
          <AuiIf condition={(s) => !messageOf(s).isCopied}>
            <CopyIcon />
      ...
      <div className="aui-user-message-content-wrapper relative col-start-2 min-w-0">
        <div className="aui-user-message-content wrap-break-word peer rounded-2xl bg-muted px-4 py-2.5 text-foreground empty:hidden">
          <MessagePrimitive.Quote>
            {(quote: QuoteInfo) => <QuoteBlock {...quote} />}
          </MessagePrimitive.Quote>
