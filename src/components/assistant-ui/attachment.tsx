...
const useAttachmentSrc = () => {
  const { file, src } = useAuiState(
    useShallow((s): { file?: File; src?: string } => {
      const { attachment } = s as unknown as AttachmentScopeState;
...
const AttachmentUI: FC = () => {
  const aui = useAui() as unknown as { attachment: { source: string } };
  const isComposer = aui.attachment.source !== "message";

  const isImage = useAuiState(
    (s) => (s as unknown as AttachmentScopeState).attachment.type === "image",
  );
  const typeLabel = useAuiState((s) => {
    const type = (s as unknown as AttachmentScopeState).attachment.type;
    switch (type) {
      case "image":
        return "Image";
      case "document":
